...
if (process.env.NODE_ENV === "DEV") {
	utilities.githubHookExpress.init(process.env.GITHUB_HOOK_SECRET, () =>
		process.exit(),
	);

	apiApp.post("/hooks/github", utilities.githubHookExpress.controller);
	utilities.logger.info("Webhook for Github available on /hooks/github", {
		tagLabel,
	});
}
...
(async () => {
	...
	apiApp.use(
		"/agenda",
		agendaMiddleware,
		Agendash(utilities.dependencyLocator.get("agenda"), {
			title: `SOLIDUS-WORKER (${process.env.NODE_ENV.substring(0, 3)})`,
		}),
	);
	...
	apiApp.listen(process.env.PORT, async () => {
		...
		utilities.logger.info("API server running", {
			tagLabel,
