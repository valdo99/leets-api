...
let GITHUB_HOOK_SECRET, restartApplicationFn;
...
const controller = async (req, res) => {
    ...
    const hash = "sha1=" + crypto.createHmac('sha1', GITHUB_HOOK_SECRET).update(JSON.stringify(req.body)).digest('hex');
    ...
    try {
        ...
        logger.debug("Pull completed", { summary: response.summary, files: response.files, tagLabel });
