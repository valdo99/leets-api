...
new utilities.express.Service(tagLabel)
    ...
    .controller(async (req, res) => {
        ...
        const user = await Users.findOne({
            ...
            $or: [{emailConfirmation: null}, {'emailConfirmation.confirmed': false}]
        });
        ...
        if (user.emailConfirmation.notificationSentDate && moment(user.emailConfirmation.notificationSentDate).add(coolDown, 'seconds').toDate() > new Date())
            return res.forbidden(i18n.__('WAIT_BEFORE_NEW_CONFIRMATION_EMAIL'));
        
        if (!user.emailConfirmation.otp)
            user.emailConfirmation.otp = otp.generate(4, {specialChars: false, upperCase: false});        
