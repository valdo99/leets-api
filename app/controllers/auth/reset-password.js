...
new utilities.express.Service(tagLabel)
    ...
    .controller(async (req, res) => {
        ...
        const user = await Users.findOne({
            email
        }).select('+resetPassword.otp +resetPassword.validTill +resetPassword.remainingAttempts');
          
        if (!user)
            return res.forbidden(i18n.__('USER_NOT_FOUND'));
              

        if (!otp || !user.resetPassword.otp || user.resetPassword.validTill < new Date())
        ...
        if (user.resetPassword.otp !== otp) {
            ...
            const updatedUser = await Users.findOneAndUpdate(
                {_id: user._id},
                {$inc: {'resetPassword.remainingAttempts': -1}}, {new: true}).select('resetPassword.remainingAttempts').lean();
  
            return res.forbidden((i18n.__('CHANGE_PASSWORD_OPERATION_WRONG_OTP', {remainingAttempts: updatedUser.resetPassword.remainingAttempts})));
