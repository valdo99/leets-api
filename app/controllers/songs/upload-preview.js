...
const getSongsByArtistInLastWeek = async (artistId) => {
	return await Post.find({
		artist: artistId,
		status: "ONLINE",
		createdAt: { $gte: new Date(moment().subtract(1, "week")) },
	});
};
...
new utilities.express.Service(tagLabel)
	...
	.controller(async (req, res) => {
		...
		if (existingPost) {
			...
			const artistPosts = await getSongsByArtistInLastWeek(
				existingPost.artist._id,
			);
			if (artistPosts.length >= MAX_SONGS_PER_ARTIST_PER_WEEK) {
				return res.forbidden(
					i18n.__("MAX_SONGS_PER_ARTIST_REACHED", {
						max: MAX_SONGS_PER_ARTIST_PER_WEEK,
					}),
				);
			}
		...
		const { name: title, preview_url, artists, album } = await getTrackData({
			id,
			token,
		});

		const { name: artistName, followers: artistFollowers } =
			await getArtistData({ id: artists[0].id, token });

		const { getArtist, getTrack } = utilities.dependencyLocator.get("spotify");

		const { playcount, artistsWithRoles, albumOfTrack } = (await getTrack(id))
			.trackUnion;

		const artistId = artistsWithRoles.items.filter(
			(el) => el.role === "MAIN",
		)[0].artist.id;
		...
		if (high_quality_image.length) { postImage = high_quality_image[0].url; }
